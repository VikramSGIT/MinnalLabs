...
class KratosFlow extends HTMLElement {
  ...
  renderBody() {
    ...
    if (!this.flow || !this.flow.ui) {
      const hint = {
        login: "Unable to load the sign-in page.",
        registration: "Unable to load the registration page.",
        verification: "Unable to load the verification page.",
        settings: "Unable to load account settings.",
      }[this.flowType] || "Unable to load this page.";
      return "<p>" + escapeHtml(hint) + "</p>";
    }
  ...
  renderDescription() {
    ...
    const desc = {
      ...
      verification: "We sent a verification code to your email. Enter it below to verify your address.",
      settings: "Update your account details below.",
    }[this.flowType];
